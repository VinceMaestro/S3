...
//creer une fonction pour checker authorize_account et get_bucket_id
    ...
    partNumber, bucketName, log, callback) {
      ...
      try {
          ...
          result = await Promise.all([this._leech(hashedStream), get_upload_part_url(this.auth, uploadId)]);
    ...
    async createMPU(Key, metaHeaders, bucketName, websiteRedirectHeader, log,
    callback) {
    ...
    async put(stream, size, keyContext, reqUids, callback) {
        ...
        try {
            await setAuthAndBucketIdOnce(this);
            let result = await Promise.all([this._leech(stream), get_upload_url(this.auth, this.bucketId)]);
