...
const download_file_by_id = require('./b2_api_js/b2_download_file_by_id');
...
const setAuthOnce = require('./b2_api_js/setAuthOnce');
...
const get_file_info = require('./b2_api_js/b2_get_file_info');
const create_bucket = require('./b2_api_js/b2_create_bucket');
...
class B2Client {
	...
	async healthcheck(location, callback) {
		...
		try {
			await setAuthOnce(this);
			result = await create_bucket(this.auth, this.b2StorageCredentials.accountId, this._b2ContainerName, bucketType);
		} catch (e) {
			err = e;
		} finally {
			if (err) {
	...
	async get(objectGetInfo, range, reqUids, callback) {
		...
		try {
			await setAuthAndBucketIdOnce(this);
			const { dataStoreVersionId } = objectGetInfo;
			result = await download_file_by_id(this.auth, dataStoreVersionId);
		} catch (e) {
			err = e;
