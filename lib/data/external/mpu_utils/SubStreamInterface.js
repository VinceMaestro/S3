...
class SubStream extends stream.PassThrough {
    constructor(options, destCloud) {
        super(options);

        this.on('stopStreamingTo' + destCloud, function stopStreaming() {
...
class SubStreamInterface {
     ...
     */
    constructor(sourceStream, destCloud) {
        this._sourceStream = sourceStream;
        this._totalLengthCounter = 0;
        this._lengthCounter = 0;
        this._subPartIndex = 0;
        this._destCloud = destCloud;
        this._currentStream = new SubStream(null, this._destCloud);
    ...
    stopStreaming(piper) {
        ...
        this._currentStream.emit('stopStreamingTo' + this._destCloud);
    }
