...
function objectPutPart(authInfo, request, streamingV4Params, log,
    ...
    return async.waterfall([
        ...
        splitter, next) => {
          console.log('\nIf data backend is backend that handles mpu (like real AWS),no need to store part info in metadata\n')
          console.log(partNumber + 'partnumber in waterfall')
        ...
        partKey, prevObjectSize, oldLocations, partInfo, next) => {
          console.log('\nStore in data backend.\n')
            ...
            if (partInfo && (partInfo.dataStoreType === 'azure' || partInfo.dataStoreType === 'b2')) {
