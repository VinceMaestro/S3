	...
	withV4(sigCfg => {
		...
		keys.forEach(key => {
		    describe(`${key.describe} size`, () => {
				...
				it('should return code 400 when testing GET with non existing bucketLocation', done => {
					s3.getObject({ Bucket: 'PleaseDontCreateALocationWithThisNameOrThisTestWillFail-' + Date.now(), Key: testKey }, (err, res) => {
				...
				it('should return code 404 when testing GET with non existing key', done => {
					s3.getObject({ Bucket: b2Location, Key: 'PleaseDontCreateAFileWithThisNameOrThisTestWillFail-' + Date.now() }, (err, res) => {
						assert.notEqual(err, null, 'Expected error but got success');
